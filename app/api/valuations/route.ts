    ...
    try {
        ...
        for (const asset of assets) {
            ...
            if (asset.method === "STRAIGHT_LINE") {
                ...
                currentBookValue = asset.purchasePrice * Math.pow((1 - (asset.rate || 0.2)), Math.floor(yearsElapsed));
            ...
            batch.push({
                sql: `
                    ...
                    WHERE id = ?
                `,
                args: [accumulatedDepreciation, currentBookValue, nowISO, asset.valuationId]
            });
...
export async function GET(req: NextRequest) {
    try {
        ...
        const valuationsRes = await db.execute(`
            ...
            LIMIT 100
        `);
        const valuations = valuationsRes.rows;
